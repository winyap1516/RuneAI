...
## 页面与流程
- 入口：`index.html`（可通过弹窗组件 `public/components/modal_login.html` 打开登录）或独立 `login.html`
- 登录：在登录表单或登录弹窗完成后 → 成功跳转 `dashboard.html`
- 注册：`register.html` → 显示“检查邮箱”提示 → 用户返回登录入口（弹窗或登录页）
- 访问控制：`dashboard.html` 加载时检查 Session，无则跳转 `index.html`
...
- `resendVerify(email)`（UI：`#resendVerifyLink`）
  - 调用 `supabase.auth.resend({ type: 'signup', email, options: { redirectTo: FRONTEND_BASE_URL + '/dashboard.html' } })`
  - 成功提示“验证邮件已重新发送”
- `onAuthStateChange(event, session)`
  - `SIGNED_IN`：保存本地用户信息 → 调用 `linkController.initSyncAfterLogin()` → `redirect: dashboard.html`
  - `SIGNED_OUT`：清理本地状态 → `redirect: index.html`
- `onAuthSuccess(user)`
...
## OAuth（Google）
- 行为：`supabase.auth.signInWithOAuth({ provider: 'google', options:{ redirectTo: FRONTEND_BASE_URL + '/dashboard.html' } })`
...
- 按钮文案：
  ...
  - 注册：`立即注册` → `注册中...`（loading）
- 链接：
  - 忘记密码：`#forgotPwdLink` → 触发 `resetPasswordForEmail`
  - 重发验证邮件：`#resendVerifyLink` → 触发 `resendVerify`
- 错误提示：
  - 登录：邮箱/密码必填 → “请输入邮箱和密码”
