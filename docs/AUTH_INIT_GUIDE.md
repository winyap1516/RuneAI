...
2.  **页面分工**：
    *   `landing/index.html`：由 `js/main.js` 负责调用 `initAuthUI('global')`。
    *   `login.html` / `register.html`：由页面内的内联脚本负责调用 `initAuthUI('login')` 或 `initAuthUI('register')`（`main.js` 会自动跳过这些页面）。
    *   `dashboard.html`：由 `js/dashboard_init.js` 负责初始化，不调用 `initAuthUI`（或仅用于非 UI 逻辑）。
